...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Generate downloadable NT/NB Ultrasonography HTML content
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="download-info">
          <strong>NT/NB Ultrasonography Report Downloaded Successfully</strong><br>
          <small>You can print this document using Ctrl+P or save as PDF</small>
        </div>
    ...
    const filename = `USG_Report_${report.patient_name.replace(/\s+/g, '_')}_${report.report_id}.html`;
    ...
    console.error('Error downloading USG report:', error);
