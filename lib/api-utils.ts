 ...
 * 
 * This file provides utility functions to help with handling API routes in Next.js
 * especially when dealing with static site generation
 */

// Add this export to the top of API route files that should always be dynamic
export const dynamic = 'force-dynamic';

/**
 * A helper function to check if the current execution is during static build
 * This function can be used to provide mock data during static site generation
 * 
 * @returns boolean - Whether the current execution is during static build
 */
 ...
 * @param mockParams - Mock params to use during static build
 * @returns URL search params
 */
export function getSearchParams(request: Request, mockParams: Record<string, string> = {}): URLSearchParams | {
  ...
  if (isStaticBuild()) {
    return {
      ...
      has: (key: string) => Object.prototype.hasOwnProperty.call(mockParams, key),
      getAll: (key: string) => mockParams[key] ? [mockParams[key]] : [],
 ...
 * @param request - The request object
 * @param mockCookies - Mock cookies to use during static build
 * @returns Cookies object
 */
export function getCookies(request: Request, mockCookies: Record<string, string> = {}): {
  get: (key: string) => { name?: string; value: string } | undefined;
  getAll: () => Array<{ name: string; value: string }>;
} {
  if (isStaticBuild()) {
    return {
      get: (key: string) => mockCookies[key] ? { value: mockCookies[key] } : undefined,
      getAll: () => Object.entries(mockCookies).map(([name, value]) => ({ name, value }))
    };
  }
  
  try {
    return (request as any).cookies;
  } catch (error) {
 ...
 * @param request - The request object
 * @param mockHeaders - Mock headers to use during static build
 * @returns Headers object
 */
export function getHeaders(request: Request, mockHeaders: Record<string, string> = {}): {
  get: (key: string) => string | null;
  has: (key: string) => boolean;
  ...
  if (isStaticBuild()) {
    return {
      ...
      has: (key: string) => Object.prototype.hasOwnProperty.call(mockHeaders, key.toLowerCase()),
 ...
 * @returns Mock data for the specified route
 */
export function getMockData(route: string): any {
...
/**
 * A helper function to provide mock data during static site generation
 * 
 * @param mockData - The mock data to return during static build
 * @returns The mock data response
 */
export function getMockDataForBuild(mockData: any) {
  return {
    success: true,
    message: "Static build mock data",
    data: mockData
  };
}